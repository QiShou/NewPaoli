//
//  PLSetPasswordViewController.swift
//  Paoli
//
...
//            loginMsg(mobile: self.accountFd.text, smsCode: self.passwordFd.text)
            setpwd(mobile: self.accountFd.text, smsCode: "\(self.mesgCodeFd.text ?? "")", password: self.passwordFd.text)
        }
...
extension PLSetPasswordViewController {
    ...
    func sendCodeMsg(mobile:String?,type:String?) {
        ...
        downBtn.start()
        let request = UserAPI.sendMsgCode(mobile: self.accountFd.text!, type:"SETTING_PWD")
    ...
    func setpwd(mobile:String?,smsCode:String?,password:String?) {
